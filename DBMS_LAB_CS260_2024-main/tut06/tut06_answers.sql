...
SELECT c.course_name, COUNT(*) AS num_students_enrolled
FROM courses c
INNER JOIN enrollments e ON c.course_id = e.course_id
GROUP BY c.course_name;

-- 9. Show the first name, last name, and age of students who have not enrolled in any courses.
SELECT s.first_name, s.last_name, s.age
FROM students s
...
-- 11. Display the first name, last name, and course name of all students who have received grades above 'B'.
SELECT s.first_name, s.last_name, c.course_name
FROM students s
INNER JOIN enrollments e ON s.student_id = e.student_id
INNER JOIN courses c ON e.course_id = c.course_id
...
-- 12. Show the course names and the names of instructors with a last name starting with 'S'.
SELECT c.course_name, i.first_name AS instructor_first_name, i.last_name AS instructor_last_name
FROM courses c
INNER JOIN instructors i ON c.instructor_id = i.instructor_id
